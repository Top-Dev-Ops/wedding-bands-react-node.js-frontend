...
class SectionControls extends Component {
    ...
    clickWindow = e => {
        if (e.target.id.includes('diamond_size') || e.target.id.includes('diamond_quality') || e.target.id.includes('num_of_stones') || e.target.id.includes('position_') || e.target.id.includes('rows_')) { return; }
        else {
    ...
    showRows = () => { this.setState({ show_rows: !this.state.show_rows }); }
    ...
    handleRows = e => {
        const value = parseInt(e.target.id.replace('rows_', ''));
        this.setState({ rows: value, show_rows: false });
        if (this.props.data.ring === 'pair') { this.props.setRingPairRows(value); }
        else if (this.props.data.ring === 'ring_1') { this.props.setRing1Rows(value); }
        else { this.props.setRing2Rows(value); }
    }
    handlePosition = e => {
    ...
    render() {
        const { show_diamond_size, show_diamond_quality, show_num_of_stones, show_rows, show_position } = this.state;
        ...
        const row = this.props.data.ring === 'pair' || this.props.data.ring === 'ring_1' ? this.props.data.ring_1_rows : this.props.data.ring_2_rows;
        ...
        let rows = [1, 2, 3, 4];
        let _rows = null;
        _rows = (this.props.data.ring === 'pair' || this.props.data.ring === 'ring_1') && this.props.data.ring_1_material === 'two_colors_3' ? [1, 2] : [1, 2, 3, 4];
        _rows = this.props.data.ring === 'ring_2' && this.props.data.ring_2_material === 'two_colors_3' ? [1, 2] : [1, 2, 3, 4];
        const profile_widths = Array.from(Array(17)).map((e, i) => i / 2 + 2);
        const diamond_widths = [1, 1, 1, 1.2, 1.2, 1.3, 1.5, 1.7, 2, 2.2, 2.4, 2.5, 2.7, 2.8, 2.9, 3, 3.2, 3.4, 3.5, 3.6, 3.7, 3.7, 3.8, 3.8, 3.9, 4, 4, 4.1, 4.1, 4.1, 4.1, 4.1, 4.2, 4.2, 4.2, 4.4, 4.4, 4.5, 4.5, 4.5];
        const profile_width = this.props.data.ring === 'pair' || this.props.data.ring === 'ring_1' ? profile_widths[this.props.data.ring_1_width] : profile_widths[this.props.data.ring_2_width];
        let row_limits = 0;
        [1, 2, 3, 4].forEach(i => { row_limits = (profile_width - 1.5) / i >= 1 ? i - 1 : row_limits });

        let positions = ['Left', 'Middle', 'Right', 'Free'];
        let _positions = ['Left', 'Middle', 'Right', 'Free'];

        const brilliant_size_limits = [0, 0, 6, 10, 10, 11, 12, ...Array(10).fill(13)];
        const princess_size_limits = [-1, -1, 0, 1, ...Array(13).fill(3)];
        const baguette_size_limits = [-1, 0, 0, 0, 1, ...Array(12).fill(3)];
        const ring_row = this.props.data.ring === 'pair' || this.props.data.ring === 'ring_1' ? this.props.data.ring_1_rows + 1 : this.props.data.ring_2_rows + 1;
        brilliant_size_limits.forEach((item, index) => {
            brilliant_size_limits.splice(index, 1, item / ring_row);
        });
        // console.log('rows: ' + ring_row, 'width: ' + profile_width, 'sizes: ', brilliant_size_limits, row_limits);
        ...
        _brilliant_diamond_sizes = _brilliant_diamond_sizes.map((item, index) => {
            return <li id={'diamond_size_' + index} key={index} className={index == diamond_size ? 'active' : index > brilliant_size_limits[ring_width] ? 'disabled' : ''} onClick={this.handleDiamondSize}>
        ...
        _princess_diamond_sizes = _princess_diamond_sizes.map((item, index) => {
            return <li id={'diamond_size_' + index} key={index} className={index == diamond_size ? 'active' : index > princess_size_limits[ring_width] ? 'disabled' : ''} onClick={this.handleDiamondSize}>
        ...
        _baguette_diamond_sizes = _baguette_diamond_sizes.map((item, index) => {
            return <li id={'diamond_size_' + index} key={index} className={index == diamond_size ? 'active' : index > baguette_size_limits[ring_width] ? 'disabled' : ''} onClick={this.handleDiamondSize}>
        ...
        _rows = _rows.map((item, index) => {
            return <li id={'rows_' + index} key={index} className={index == row ? 'active' : index > row_limits ? 'disabled' : ''} onClick={this.handleRows}>
                <span id={'rows_' + index} className="option-label">{item}</span>
            </li>
        });
        ...
        return (
            <div className="bezel-controls">
                <div className="row justify-content-start">
                    ...
                    {/* rows */}
                    <div className="col-sm-4 diamond-quality rows-class">
                        <label>Rows</label>
                        <div className="diamond-size-select">
                            {/* select */}
                            <div className="select-toggle" onClick={this.showRows}>
                                <span className="option-label" onClick={this.showRows}>{rows[parseInt(row)]}</span>
                                <i icon="dropdown-arrow" className="svg-icon svg-icon-dropdown-arrow">
                                    <svg id="dropdown-arrow" viewBox="0 0 18 18" width="18" height="18" onClick={this.showRows}>
                                        <path d="M 3 7.3266 L 9 13 L 15 7.3507 L 13.7368 6 L 9 10.5773 L 4.2632 6 L 3 7.3266 Z"></path>
                                    </svg>
                                </i>
                            </div>
                            {/* select options */}
                            <div className="select-options" style={{ display: `${show_rows ? 'block' : 'none'}` }}>
                                <ul> {_rows} </ul>
                            </div>
                        </div>
                    </div>
...
SectionControls.propTypes = { data: PropTypes.object.isRequired };
...
export default connect(mapStateToProps, mapActionsToProps)(SectionControls);